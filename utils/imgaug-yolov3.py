...
num_outfiles = 40
infile = 'F:/7_pla/cry/add'
output_path = 'F:/7_pla/cry/addd'
img_file_names = os.listdir(infile)
for file_name in img_file_names:
    if os.path.splitext(file_name)[1] == ".jpg":
        img_path = os.path.join(infile, file_name.split('.')[0] + '.jpg')
        print('开始数据增强处理，增幅倍数：', num_outfiles)
        print('正在处理图片：', file_name)
        ## load 'infile' image into numpy array 'num_outfiles' times...
        img = imageio.imread(img_path)  # read you image
        height, width, depth = img.shape
        ...
        for idx, image in enumerate(images):
            ...
            outfile = open(output_path + '/' + str(idx) + '-' + name + '.txt', 'w')
            ...
            if os.path.getsize(outfile.name) > 0:
                ...
                imageio.imwrite(output_path + '/' + str(idx) + '-' + filename,
                                image_aug)  # write all changed images
            else:
